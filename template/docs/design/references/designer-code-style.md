...
In this project the design system master lives in `docs/design/system.md` (tokens, global rules, anti-patterns) and page-specific specs live in `docs/design/pages/`.

```
docs/design/
├── system.md              # Tokens, global rules, anti-patterns (source of truth)
├── principles.md
├── decisions/             # ADRs
...
`docs/design/system.md` is the single source of truth for tokens. Page specs reference tokens by name — never by raw value.

---

## Page Spec Structure

Every page spec in `docs/design/pages/` follows this structure:
...
## Accessibility Notes
Anything beyond the global baseline (`docs/design/system.md`) that this page requires.
```

**Rules:**
- One page file = one route/view
- Sections ordered top-to-bottom as they appear on the page
- Every section references tokens from `docs/design/system.md` — never raw hex or pixel values
- If a needed token doesn't exist in `docs/design/system.md`, add it there first
...
- All values use semantic tokens from `docs/design/system.md`
...
Tokens live in `docs/design/system.md` and map 1:1 to Tailwind v4 `@theme` in `frontend/src/styles/globals.css`.

**Rules:**
- Never use raw hex values (`#3b82f6`) in specs — always token names (`primary`)
- Never use arbitrary pixel values (`437px`) — use the spacing scale from `docs/design/system.md`
- Never use arbitrary shadows or z-index — use the elevation/layering scale
- If a design needs a value not in `docs/design/system.md` → add the token there first, then reference it
...
- Specify tone consistent with the "Domain & Tone" section of `docs/design/system.md`
...
Beyond the `docs/design/system.md` baseline (contrast, touch targets, focus rings):
...
- [ ] All tokens reference `docs/design/system.md` — zero raw hex/pixel values
...
- [ ] New tokens (if any) added to `docs/design/system.md` first
...
- No arbitrary pixel values — use the spacing/sizing scale from `docs/design/system.md`
...
- No design changes without updating `docs/design/system.md` if new tokens are needed
